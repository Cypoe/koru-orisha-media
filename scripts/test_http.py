...
def main() -> int:
    ...
    if mode == "fixture":
        ...
        check("hero-item-link" in hero_slide and "{{id}}" in hero_slide, "hero-slide fragment is slotted")
        ...
        ra = body.find(b"<h2>Recently added</h2>")
        mv = body.find(b"<h2>Movies</h2>")
        se = body.find(b"<h2>Series</h2>")
        check(ra != -1 and mv != -1 and se != -1 and ra < mv < se, "home shelves: recent then each mount")
        ...
        check(b"hero-item-link" in body, "home hero art links to the item")
