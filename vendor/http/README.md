...
| `run-accept-loop` + `streamsBusy()` | Zig serve path + idle exit (`KORU_IDLE_SECS`) |

**Producer** of `STREAM:v1` stays in `src/consumers.kz` (`mediaHttp`). This module only **consumes** it.

## Orisha is separate

[`vendor/orisha/`](../orisha/) is a verbatim copy of `W:\src\orisha\lib`. Refresh it with:
...
When that lands, delete this module's listen/accept/send loop and keep only the STREAM producer in `src/`.
